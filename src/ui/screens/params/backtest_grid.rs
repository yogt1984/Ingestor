//! Backtest Grid Config Screen (T-2.11)
//!
//! TUI screen for configuring backtest grid command parameters (MM only).
...
use crate::commands::params::backtest_params::{GridParams, GridParamsBuilder};
...
#[derive(Debug, Clone)]
pub struct BacktestGridConfigScreen {
    pub current_group: ParameterGroup,
    pub selected_field_index: usize,
    pub widgets: HashMap<GridField, FieldWidget>,
    pub validation_errors: HashMap<GridField, String>,
    pub total_combinations: usize,
}

impl Default for BacktestGridConfigScreen {
    fn default() -> Self {
        Self::new()
    }
}
...
impl BacktestGridConfigScreen {
    ...
    fn initialize_widgets(&mut self) {
        self.widgets.insert(GridField::DataPath, FieldWidget::PathInput(
            PathInputWidget::new()
                .with_placeholder("Path to data directory...")
                .set_focused(false)
        ));
        
        self.widgets.insert(GridField::Algorithm, FieldWidget::Dropdown(
            DropdownWidget::new()
        ...
        self.widgets.insert(GridField::WeightsFile, FieldWidget::PathInput(
            PathInputWidget::new()
                .with_placeholder("Path to weights file (optional)...")
                .set_focused(false)
        ));
        
        self.widgets.insert(GridField::Spreads, FieldWidget::CommaList(
            CommaListWidget::new()
                .with_placeholder("e.g., 1.0,2.0,3.0")
                .set_focused(false)
        ));
        
        self.widgets.insert(GridField::Skews, FieldWidget::CommaList(
            CommaListWidget::new()
                .with_placeholder("e.g., 0.3,0.5,0.7")
                .set_focused(false)
        ));
        
        self.widgets.insert(GridField::MaxInventory, FieldWidget::NumberInput(
        ...
        self.widgets.insert(GridField::QuoteSize, FieldWidget::NumberInput(
            NumberInputWidget::new()
                .with_value(0.001)
                .with_min(0.0)
                .with_decimals(4)
        ...
        self.widgets.insert(GridField::FeeRate, FieldWidget::NumberInput(
            NumberInputWidget::new()
                .with_value(0.0001)
                .with_min(0.0)
                .with_decimals(6)
        ...
        self.widgets.insert(GridField::NaiveFills, FieldWidget::Toggle(
            ToggleWidget::new()
                .with_value(false)
        ...
        self.widgets.insert(GridField::FillProb, FieldWidget::NumberInput(
            NumberInputWidget::new()
                .with_value(0.1)
                .with_min(0.0)
                .with_max(1.0)
        ...
        self.widgets.insert(GridField::QueuePos, FieldWidget::NumberInput(
            NumberInputWidget::new()
                .with_value(0.5)
                .with_min(0.0)
                .with_max(1.0)
        ...
        self.widgets.insert(GridField::Quiet, FieldWidget::Toggle(
            ToggleWidget::new()
                .with_value(false)
        ...
        self.widgets.insert(GridField::Output, FieldWidget::PathInput(
            PathInputWidget::new()
    ...
    pub fn current_field(&self) -> Option<GridField> {
        let fields = GridField::fields_in_group(self.current_group);
        fields.get(self.selected_field_index).copied()
    }

    pub fn get_widget(&self, field: GridField) -> Option<&FieldWidget> {
        self.widgets.get(&field)
    }

    pub fn get_widget_mut(&mut self, field: GridField) -> Option<&mut FieldWidget> {
            ...
            _ => {
                if let Some(field) = self.current_field() {
                    if let Some(widget) = self.get_widget_mut(field) {
                        ...
                        if handled {
                            if matches!(field, GridField::Spreads | GridField::Skews) {
    ...
    fn update_combinations(&mut self) {
        let spreads_count = self.get_grid_size(GridField::Spreads);
        let skews_count = self.get_grid_size(GridField::Skews);
        self.total_combinations = spreads_count * skews_count;
    }

    fn get_grid_size(&self, field: GridField) -> usize {
    ...
    pub fn build_params(&self) -> Result<GridParams> {
        let mut builder = GridParamsBuilder::new();

        if let Some(FieldWidget::PathInput(w)) = self.widgets.get(&GridField::DataPath) {
            if !w.path().is_empty() {
                builder = builder.data_path(PathBuf::from(w.path()));
            }
        }

        if let Some(FieldWidget::Dropdown(w)) = self.widgets.get(&GridField::Algorithm) {
            if let Some(alg) = w.selected_option() {
                builder = builder.algorithm(alg.clone());
            }
        }

        if let Some(FieldWidget::PathInput(w)) = self.widgets.get(&GridField::WeightsFile) {
            if !w.path().is_empty() {
                builder = builder.weights_file(Some(PathBuf::from(w.path())));
            }
        }

        if let Some(FieldWidget::CommaList(w)) = self.widgets.get(&GridField::Spreads) {
            builder = builder.spreads(w.values().iter().map(|v| v.to_string()).collect::<Vec<_>>().join(","));
        }

        if let Some(FieldWidget::CommaList(w)) = self.widgets.get(&GridField::Skews) {
            builder = builder.skews(w.values().iter().map(|v| v.to_string()).collect::<Vec<_>>().join(","));
        }

        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&GridField::MaxInventory) {
            builder = builder.max_inventory(w.value());
        }

        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&GridField::QuoteSize) {
            builder = builder.quote_size(w.value());
        }

        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&GridField::FeeRate) {
            builder = builder.fee_rate(w.value());
        }

        if let Some(FieldWidget::Toggle(w)) = self.widgets.get(&GridField::NaiveFills) {
            builder = builder.naive_fills(w.value());
        }

        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&GridField::FillProb) {
            builder = builder.fill_prob(w.value());
        }

        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&GridField::QueuePos) {
            builder = builder.queue_pos(w.value());
        }

        if let Some(FieldWidget::Toggle(w)) = self.widgets.get(&GridField::Quiet) {
            builder = builder.quiet(w.value());
        }

        if let Some(FieldWidget::PathInput(w)) = self.widgets.get(&GridField::Output) {
            if !w.path().is_empty() {
    ...
    pub fn validate(&mut self) {
        ...
        if let Some(FieldWidget::PathInput(w)) = self.widgets.get(&GridField::DataPath) {
            if w.path().is_empty() {
                self.validation_errors.insert(GridField::DataPath, "Data path is required".to_string());
            }
        }

        if let Some(FieldWidget::Dropdown(w)) = self.widgets.get(&GridField::Algorithm) {
            if w.selected_option().is_none() {
                self.validation_errors.insert(GridField::Algorithm, "Algorithm is required".to_string());
            } else if let Some(alg) = w.selected_option() {
                if !["as", "ml", "fixed"].contains(&alg.as_str()) {
                    self.validation_errors.insert(GridField::Algorithm, "Algorithm must be MM type: as, ml, or fixed".to_string());
                }
            }
        }

        for field in [GridField::Spreads, GridField::Skews] {
            if let Some(FieldWidget::CommaList(w)) = self.widgets.get(&field) {
                if w.is_empty() {
        ...
        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&GridField::FillProb) {
            if w.value() < 0.0 || w.value() > 1.0 {
                self.validation_errors.insert(GridField::FillProb, "Fill probability must be in range [0.0, 1.0]".to_string());
            }
        }

        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&GridField::QueuePos) {
            if w.value() < 0.0 || w.value() > 1.0 {
                self.validation_errors.insert(GridField::QueuePos, "Queue position must be in range [0.0, 1.0]".to_string());
            }
        }
    }
...
pub fn draw_backtest_grid_config_screen(
    f: &mut Frame,
    screen: &BacktestGridConfigScreen,
) {
    let area = f.area();
...
fn draw_content_area(f: &mut Frame, area: Rect, screen: &BacktestGridConfigScreen) {
    let fields = GridField::fields_in_group(screen.current_group);
    if fields.is_empty() {
        return;
    }
...
fn draw_preview_and_status(f: &mut Frame, area: Rect, screen: &BacktestGridConfigScreen) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(2),
...
mod tests {
    ...
    #[test]
    fn test_new_screen() {
        let screen = BacktestGridConfigScreen::new();
        assert_eq!(screen.current_group, ParameterGroup::Basic);
    }

    #[test]
    fn test_grid_field_all() {
        let fields = GridField::all();
        assert_eq!(fields.len(), 13);
    }

    #[test]
    fn test_build_params_with_required() {
        let mut screen = BacktestGridConfigScreen::new();
        
        if let Some(FieldWidget::PathInput(ref mut w)) = screen.get_widget_mut(GridField::DataPath) {
            let new_w = w.clone().with_path("/tmp/data");
            *w = new_w;
        }
        if let Some(FieldWidget::Dropdown(ref mut w)) = screen.get_widget_mut(GridField::Algorithm) {
