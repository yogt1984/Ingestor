//! Backtest Sweep Config Screen (T-2.11)
//!
//! TUI screen for configuring backtest sweep command parameters (both algorithm types).
...
use crate::commands::params::backtest_params::{SweepParams, SweepParamsBuilder};
...
impl SweepField {
    ...
    pub fn group(&self) -> ParameterGroup {
        match self {
            ...
            Self::Spreads | Self::Skews | Self::MaxInventory | Self::QuoteSize |
...
#[derive(Debug, Clone)]
pub struct BacktestSweepConfigScreen {
    pub current_group: ParameterGroup,
    pub selected_field_index: usize,
    pub widgets: HashMap<SweepField, FieldWidget>,
    pub validation_errors: HashMap<SweepField, String>,
    pub total_combinations: usize,
}

impl Default for BacktestSweepConfigScreen {
    fn default() -> Self {
        Self::new()
    }
}
...
impl BacktestSweepConfigScreen {
    pub fn new() -> Self {
        let mut screen = Self {
    ...
    fn initialize_widgets(&mut self) {
        self.widgets.insert(SweepField::DataPath, FieldWidget::PathInput(
            PathInputWidget::new()
                .with_placeholder("Path to data directory...")
                .set_focused(false)
        ));
        
        self.widgets.insert(SweepField::Algorithm, FieldWidget::Dropdown(
            DropdownWidget::new()
        ...
        self.widgets.insert(SweepField::WeightsFile, FieldWidget::PathInput(
            PathInputWidget::new()
                .with_placeholder("Path to weights file (optional)...")
                .set_focused(false)
        ));
        
        self.widgets.insert(SweepField::Spreads, FieldWidget::CommaList(
            CommaListWidget::new()
                .with_placeholder("e.g., 1.0,2.0,3.0")
                .set_focused(false)
        ));
        
        self.widgets.insert(SweepField::Skews, FieldWidget::CommaList(
            CommaListWidget::new()
                .with_placeholder("e.g., 0.3,0.5,0.7")
                .set_focused(false)
        ));
        
        self.widgets.insert(SweepField::MaxInventory, FieldWidget::NumberInput(
        ...
        self.widgets.insert(SweepField::QuoteSize, FieldWidget::NumberInput(
            NumberInputWidget::new()
                .with_value(0.001)
                .with_min(0.0)
                .with_decimals(4)
        ...
        self.widgets.insert(SweepField::FeeRate, FieldWidget::NumberInput(
            NumberInputWidget::new()
                .with_value(0.0001)
                .with_min(0.0)
                .with_decimals(6)
        ...
        self.widgets.insert(SweepField::NaiveFills, FieldWidget::Toggle(
            ToggleWidget::new()
                .with_value(false)
        ...
        self.widgets.insert(SweepField::FillProb, FieldWidget::NumberInput(
            NumberInputWidget::new()
                .with_value(0.1)
                .with_min(0.0)
                .with_max(1.0)
        ...
        self.widgets.insert(SweepField::QueuePos, FieldWidget::NumberInput(
            NumberInputWidget::new()
                .with_value(0.5)
                .with_min(0.0)
                .with_max(1.0)
        ...
        self.widgets.insert(SweepField::Quiet, FieldWidget::Toggle(
            ToggleWidget::new()
                .with_value(false)
                .with_label("Quiet mode (no progress output)")
                .set_focused(false)
        ));
        
        self.widgets.insert(SweepField::Output, FieldWidget::PathInput(
            PathInputWidget::new()
    ...
    pub fn current_field(&self) -> Option<SweepField> {
        let fields = SweepField::fields_in_group(self.current_group);
        fields.get(self.selected_field_index).copied()
    }

    pub fn get_widget(&self, field: SweepField) -> Option<&FieldWidget> {
        self.widgets.get(&field)
    }

    pub fn get_widget_mut(&mut self, field: SweepField) -> Option<&mut FieldWidget> {
            ...
            _ => {
                if let Some(field) = self.current_field() {
                    if let Some(widget) = self.get_widget_mut(field) {
                        ...
                        if handled {
                            if matches!(field, SweepField::Spreads | SweepField::Skews) {
    ...
    fn update_combinations(&mut self) {
        let spreads_count = self.get_grid_size(SweepField::Spreads);
        let skews_count = self.get_grid_size(SweepField::Skews);
        self.total_combinations = spreads_count * skews_count;
    }

    fn get_grid_size(&self, field: SweepField) -> usize {
    ...
    pub fn build_params(&self) -> Result<SweepParams> {
        let mut builder = SweepParamsBuilder::new();

        if let Some(FieldWidget::PathInput(w)) = self.widgets.get(&SweepField::DataPath) {
            if !w.path().is_empty() {
                builder = builder.data_path(PathBuf::from(w.path()));
            }
        }

        if let Some(FieldWidget::Dropdown(w)) = self.widgets.get(&SweepField::Algorithm) {
            if let Some(alg) = w.selected_option() {
                builder = builder.algorithm(alg.clone());
            }
        }

        if let Some(FieldWidget::PathInput(w)) = self.widgets.get(&SweepField::WeightsFile) {
            if !w.path().is_empty() {
                builder = builder.weights_file(Some(PathBuf::from(w.path())));
            }
        }

        if let Some(FieldWidget::CommaList(w)) = self.widgets.get(&SweepField::Spreads) {
            builder = builder.spreads(w.values().iter().map(|v| v.to_string()).collect::<Vec<_>>().join(","));
        }

        if let Some(FieldWidget::CommaList(w)) = self.widgets.get(&SweepField::Skews) {
            builder = builder.skews(w.values().iter().map(|v| v.to_string()).collect::<Vec<_>>().join(","));
        }

        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&SweepField::MaxInventory) {
            builder = builder.max_inventory(w.value());
        }

        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&SweepField::QuoteSize) {
            builder = builder.quote_size(w.value());
        }

        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&SweepField::FeeRate) {
            builder = builder.fee_rate(w.value());
        }

        if let Some(FieldWidget::Toggle(w)) = self.widgets.get(&SweepField::NaiveFills) {
            builder = builder.naive_fills(w.value());
        }

        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&SweepField::FillProb) {
            builder = builder.fill_prob(w.value());
        }

        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&SweepField::QueuePos) {
            builder = builder.queue_pos(w.value());
        }

        if let Some(FieldWidget::Toggle(w)) = self.widgets.get(&SweepField::Quiet) {
            builder = builder.quiet(w.value());
        }

        if let Some(FieldWidget::PathInput(w)) = self.widgets.get(&SweepField::Output) {
            if !w.path().is_empty() {
    ...
    pub fn validate(&mut self) {
        ...
        if let Some(FieldWidget::PathInput(w)) = self.widgets.get(&SweepField::DataPath) {
            if w.path().is_empty() {
                self.validation_errors.insert(SweepField::DataPath, "Data path is required".to_string());
            }
        }

        if let Some(FieldWidget::Dropdown(w)) = self.widgets.get(&SweepField::Algorithm) {
            if w.selected_option().is_none() {
                self.validation_errors.insert(SweepField::Algorithm, "Algorithm is required".to_string());
            }
        }

        for field in [SweepField::Spreads, SweepField::Skews] {
            if let Some(FieldWidget::CommaList(w)) = self.widgets.get(&field) {
                if w.is_empty() {
        ...
        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&SweepField::FillProb) {
            if w.value() < 0.0 || w.value() > 1.0 {
                self.validation_errors.insert(SweepField::FillProb, "Fill probability must be in range [0.0, 1.0]".to_string());
            }
        }

        if let Some(FieldWidget::NumberInput(w)) = self.widgets.get(&SweepField::QueuePos) {
            if w.value() < 0.0 || w.value() > 1.0 {
                self.validation_errors.insert(SweepField::QueuePos, "Queue position must be in range [0.0, 1.0]".to_string());
            }
        }
    }
...
pub fn draw_backtest_sweep_config_screen(
    f: &mut Frame,
    screen: &BacktestSweepConfigScreen,
) {
    let area = f.area();
...
fn draw_content_area(f: &mut Frame, area: Rect, screen: &BacktestSweepConfigScreen) {
    let fields = SweepField::fields_in_group(screen.current_group);
    if fields.is_empty() {
        return;
    }
...
fn draw_preview_and_status(f: &mut Frame, area: Rect, screen: &BacktestSweepConfigScreen) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(2),
...
mod tests {
    ...
    #[test]
    fn test_new_screen() {
        let screen = BacktestSweepConfigScreen::new();
        assert_eq!(screen.current_group, ParameterGroup::Basic);
    }

    #[test]
    fn test_sweep_field_all() {
        let fields = SweepField::all();
        assert_eq!(fields.len(), 13);
    }

    #[test]
    fn test_build_params_with_required() {
        let mut screen = BacktestSweepConfigScreen::new();
        
        if let Some(FieldWidget::PathInput(ref mut w)) = screen.get_widget_mut(SweepField::DataPath) {
            let new_w = w.clone().with_path("/tmp/data");
            *w = new_w;
        }
        if let Some(FieldWidget::Dropdown(ref mut w)) = screen.get_widget_mut(SweepField::Algorithm) {
