//! Backtest Grid Results Screen (T-3.8)
//!
//! TUI screen for displaying backtest grid command results.
...
use crate::commands::backtest::{GridResult, GridResultItem};
...
impl GridViewMode {
    /// Get all view modes
    pub fn all() -> Vec<GridViewMode> {
        vec![
            GridViewMode::TopResults,
            GridViewMode::FullTable,
            GridViewMode::Heatmap,
        ]
    }
    ...
    pub fn name(&self) -> &'static str {
        match self {
            GridViewMode::TopResults => "Top Results",
            GridViewMode::FullTable => "Full Table",
            GridViewMode::Heatmap => "Heatmap",
        }
    }

    /// Get next view mode
    pub fn next(&self) -> GridViewMode {
        let all = Self::all();
    ...
    pub fn previous(&self) -> GridViewMode {
        let all = Self::all();
        let current_idx = all.iter().position(|v| v == self).unwrap_or(0);
...
/// Backtest grid results screen
pub struct BacktestGridResultsScreen {
    /// Grid result data
    result: GridResult,
    /// Current view mode
...
impl BacktestGridResultsScreen {
    /// Create a new results screen from GridResult
    pub fn new(result: GridResult) -> Self {
        Self {
            result,
            view_mode: GridViewMode::TopResults,
            selected_index: None,
            focused: true,
            export_path: None,
        }
    ...
    /// Get the result data
    pub fn result(&self) -> &GridResult {
        &self.result
    }

    /// Get current view mode
    pub fn view_mode(&self) -> GridViewMode {
        self.view_mode
    }

    /// Set view mode
    pub fn set_view_mode(&mut self, mode: GridViewMode) {
        self.view_mode = mode;
    }
    ...
    /// Get selected result
    pub fn selected_result(&self) -> Option<&GridResultItem> {
        self.selected_index
            .and_then(|idx| self.result.all_results.get(idx))
    }

    /// Get best result
    pub fn best_result(&self) -> Option<&GridResultItem> {
    ...
    /// Get top N results
    pub fn top_results(&self, n: usize) -> Vec<&GridResultItem> {
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> bool {
        ...
        match key.code {
            ...
            KeyCode::Down | KeyCode::Char('j') => {
                if self.view_mode == GridViewMode::FullTable {
    ...
    fn create_results_table(&self, results: &[&GridResultItem]) -> (Vec<TableHeader>, Vec<TableRow>) {
        let headers = vec![
    ...
    fn create_heatmap_series(&self) -> DataSeries {
        ...
        DataSeries::new("Grid Results".to_string())
            .with_points(points)
    ...
    pub fn render(&self, f: &mut Frame, area: Rect) {
        ...
        // Render tabs
        let tab_titles: Vec<Line> = GridViewMode::all()
            .iter()
            .map(|mode| {
        ...
        let tabs = Tabs::new(tab_titles)
            ...
            .select(GridViewMode::all().iter().position(|m| *m == self.view_mode).unwrap_or(0))
            .divider("|");
    ...
    fn render_full_table(&self, f: &mut Frame, area: Rect) {
        let all: Vec<&GridResultItem> = self.result.all_results.iter().collect();
...
mod tests {
    ...
    fn create_test_grid_result() -> GridResult {
        let mut all_results = Vec::new();
        for i in 0..20 {
            all_results.push(GridResultItem {
    ...
    fn test_view_mode_all() {
        let modes = GridViewMode::all();
        assert_eq!(modes.len(), 3);
    }

    #[test]
    fn test_view_mode_name() {
        assert_eq!(GridViewMode::TopResults.name(), "Top Results");
        assert_eq!(GridViewMode::FullTable.name(), "Full Table");
        assert_eq!(GridViewMode::Heatmap.name(), "Heatmap");
    }

    #[test]
    fn test_view_mode_next() {
        assert_eq!(GridViewMode::TopResults.next(), GridViewMode::FullTable);
        assert_eq!(GridViewMode::FullTable.next(), GridViewMode::Heatmap);
        assert_eq!(GridViewMode::Heatmap.next(), GridViewMode::TopResults);
    }
    ...
    fn test_screen_creation() {
        ...
        assert_eq!(screen.view_mode(), GridViewMode::TopResults);
    ...
    fn test_set_view_mode() {
        let result = create_test_grid_result();
        let mut screen = BacktestGridResultsScreen::new(result);
        screen.set_view_mode(GridViewMode::FullTable);
        assert_eq!(screen.view_mode(), GridViewMode::FullTable);
    }
    ...
    #[test]
    fn test_best_result() {
        let mut result = create_test_grid_result();
    ...
    fn test_view_mode_cycle() {
        let mut mode = GridViewMode::TopResults;
