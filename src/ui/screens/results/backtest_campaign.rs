//! Backtest Campaign Results Screen (T-3.8)
//!
//! TUI screen for displaying backtest campaign command results.
...
use crate::commands::backtest::CampaignResult;
...
/// View mode for campaign results display
...
impl CampaignViewMode {
    /// Get all view modes
    pub fn all() -> Vec<CampaignViewMode> {
        vec![
            CampaignViewMode::Summary,
            CampaignViewMode::Weekly,
            CampaignViewMode::Metrics,
            CampaignViewMode::Verdict,
        ]
    }
    ...
    pub fn name(&self) -> &'static str {
        match self {
            CampaignViewMode::Summary => "Summary",
            CampaignViewMode::Weekly => "Weekly",
            CampaignViewMode::Metrics => "Metrics",
            CampaignViewMode::Verdict => "Verdict",
        }
    }

    /// Get next view mode
    pub fn next(&self) -> CampaignViewMode {
        let all = Self::all();
    ...
    pub fn previous(&self) -> CampaignViewMode {
        let all = Self::all();
        let current_idx = all.iter().position(|v| v == self).unwrap_or(0);
...
/// Backtest campaign results screen
pub struct BacktestCampaignResultsScreen {
...
impl BacktestCampaignResultsScreen {
    /// Create a new results screen from CampaignResult
    pub fn new(result: CampaignResult) -> Self {
        Self {
            result,
            view_mode: CampaignViewMode::Summary,
            selected_index: None,
            focused: true,
            export_path: None,
        }
    ...
    pub fn view_mode(&self) -> CampaignViewMode {
        self.view_mode
    }

    /// Set view mode
    pub fn set_view_mode(&mut self, mode: CampaignViewMode) {
        self.view_mode = mode;
    }
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> bool {
        ...
        match key.code {
            ...
            KeyCode::Down | KeyCode::Char('j') => {
                if self.view_mode == CampaignViewMode::Weekly {
    ...
    pub fn render(&self, f: &mut Frame, area: Rect) {
        ...
        // Render tabs
        let tab_titles: Vec<Line> = CampaignViewMode::all()
            .iter()
            .map(|mode| {
        ...
        let tabs = Tabs::new(tab_titles)
            ...
            .select(CampaignViewMode::all().iter().position(|m| *m == self.view_mode).unwrap_or(0))
            .divider("|");
...
mod tests {
    ...
    fn create_test_campaign_result() -> CampaignResult {
    ...
    fn test_view_mode_all() {
        let modes = CampaignViewMode::all();
        assert_eq!(modes.len(), 4);
    }

    #[test]
    fn test_view_mode_name() {
        assert_eq!(CampaignViewMode::Summary.name(), "Summary");
        assert_eq!(CampaignViewMode::Weekly.name(), "Weekly");
        assert_eq!(CampaignViewMode::Metrics.name(), "Metrics");
        assert_eq!(CampaignViewMode::Verdict.name(), "Verdict");
    }

    #[test]
    fn test_view_mode_next() {
        assert_eq!(CampaignViewMode::Summary.next(), CampaignViewMode::Weekly);
        assert_eq!(CampaignViewMode::Weekly.next(), CampaignViewMode::Metrics);
        assert_eq!(CampaignViewMode::Metrics.next(), CampaignViewMode::Verdict);
        assert_eq!(CampaignViewMode::Verdict.next(), CampaignViewMode::Summary);
    }

    #[test]
    fn test_screen_creation() {
        let result = create_test_campaign_result();
        let screen = BacktestCampaignResultsScreen::new(result);
        assert_eq!(screen.view_mode(), CampaignViewMode::Summary);
    ...
    fn test_set_view_mode() {
        let result = create_test_campaign_result();
        let mut screen = BacktestCampaignResultsScreen::new(result);
        screen.set_view_mode(CampaignViewMode::Weekly);
        assert_eq!(screen.view_mode(), CampaignViewMode::Weekly);
    }
    ...
    fn test_format_verdict() {
        assert_eq!(BacktestCampaignResultsScreen::format_verdict(&ValidationVerdict::GoLive), "Go Live");
        assert_eq!(BacktestCampaignResultsScreen::format_verdict(&ValidationVerdict::Recalibrate), "Recalibrate");
    }
    ...
    fn test_view_mode_cycle() {
        let mut mode = CampaignViewMode::Summary;
